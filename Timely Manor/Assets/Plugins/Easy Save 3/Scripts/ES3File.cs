...
public class ES3File
{
    ...
    public byte[] LoadRawBytes(ES3Settings settings = null)
    {
        if (settings == null)
            settings = this.settings;

        return GetBytes(settings, true);
    }
    ...
    public string LoadRawString(ES3Settings settings = null)
    {
        if (cache.Count == 0)
            return "";
        return settings.encoding.GetString(LoadRawBytes(settings));
    }

    /*
     * Same as LoadRawString, except it will return an encrypted/compressed file if these are enabled.
     */
    internal byte[] GetBytes(ES3Settings settings = null, bool postProcess = false)
    {
        if (cache.Count == 0)
            return new byte[0];
